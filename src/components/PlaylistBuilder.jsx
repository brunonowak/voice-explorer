...
function PlaylistBuilder({ token, userId, coaches, winners = [], countryName, onClose }) {
  const allArtists = [...coaches, ...winners.map(w => w.name)];
  const [playlistName, setPlaylistName] = useState(() => buildDefaultName(allArtists, countryName));
  ...
  const effectivePerCoach = trackMode === 'total'
    ? Math.max(1, Math.ceil(totalTracks / allArtists.length))
    : tracksPerCoach;
  ...
  const buildPlaylist = async () => {
    ...
    try {
      ...
      // Fetch winner tracks (treated like regular artists)
      for (const winner of winners) {
        setProgress(`Searching for ${winner.name} 🏆...`);
        const artists = await searchArtist(token, winner.name, spotifyOverrides);

        if (artists.length === 0) {
          skipped.push(`${winner.name} 🏆`);
          continue;
        }
        ...
        setProgress(
          needsExpanded
            ? `Loading discography for ${winner.name} 🏆...`
            : `Getting top tracks for ${winner.name} 🏆...`
        );
      ...
      const allNames = [...coaches, ...winners.map(w => `${w.name} 🏆`)];
      const mixLabel = MIX_OPTIONS.find(m => m.value === mixType)?.label ?? '';
      const playlist = await createPlaylist(
        token, userId, playlistName,
        `${mixLabel} mix — ${allNames.join(', ')} · Generated by Voice Explorer`,
        isPublic
      );
