...
function selectVideos(videos, channelId, { tracksPerCoach, mixType, artistName }) {
  ...
  if (mixType === 'fresh-hits' || mixType === 'oldies') {
    // Split pool at median publish date, then sort each half by views
    const withDate = pool.filter(v => v.publishedAt).sort((a, b) =>
      new Date(a.publishedAt) - new Date(b.publishedAt)
    );
    const midpoint = Math.floor(withDate.length / 2);
    const half = mixType === 'fresh-hits'
      ? withDate.slice(midpoint)   // newer half
      : withDate.slice(0, midpoint); // older half
    half.sort((a, b) => (b.viewCount ?? 0) - (a.viewCount ?? 0));
    // If not enough in the target half, fill from the other side
    if (half.length < tracksPerCoach) {
      const otherHalf = mixType === 'fresh-hits'
        ? withDate.slice(0, midpoint)
        : withDate.slice(midpoint);
      otherHalf.sort((a, b) => (b.viewCount ?? 0) - (a.viewCount ?? 0));
      const usedIds = new Set(half.map(v => v.id));
      for (const v of otherHalf) {
        if (!usedIds.has(v.id)) half.push(v);
        if (half.length >= tracksPerCoach) break;
      }
    }
    return half.slice(0, tracksPerCoach);
  }
  ...
  // Balanced (Spotify default): mix of high/mid/low view counts
  pool.sort((a, b) => (b.viewCount ?? 0) - (a.viewCount ?? 0));
  const third = Math.max(1, Math.ceil(tracksPerCoach / 3));
...
function PlaylistBuilder({ token, userId, coaches, countryName, onClose, platform }) {
  ...
  const buildPlaylist = async () => {
    ...
    try {
      ...
      const allMixOptions = isYouTube ? MIX_OPTIONS_YOUTUBE : MIX_OPTIONS;
      const mixLabel = allMixOptions.find(m => m.value === mixType)?.label ?? '';
